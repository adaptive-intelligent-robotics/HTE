...
template<typename NumericX, typename NumericY, typename NumericU, typename NumericW, typename NumericZ>
bool quiver(const std::vector<NumericX>& x, const std::vector<NumericY>& y,
            ...
            const std::vector<NumericZ>& z,const std::map<std::string, std::string>& keywords = {})
{
    assert(x.size() == y.size() && x.size() == u.size() && u.size() == w.size());
    ...
    PyObject* plot_args = PyTuple_New(5);
    PyTuple_SetItem(plot_args, 0, xarray);
    PyTuple_SetItem(plot_args, 1, yarray);
    PyTuple_SetItem(plot_args, 2, uarray);
    PyTuple_SetItem(plot_args, 3, warray);
    PyTuple_SetItem(plot_args, 4, zarray);

    // construct keyword args
    PyObject* kwargs = PyDict_New();
    // PyDict_SetItemString(kwargs, "c", zarray);
    // PyDict_SetItemString(kwargs, "color", "g");
 
    // PyObject *python_colormap = airl_colormap();
    
    // PyDict_SetItemString(kwargs, "color", python_colormap);
