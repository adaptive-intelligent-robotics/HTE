#ifndef STAT_QD_MAP_VEL_HPP_
#define STAT_QD_MAP_VEL_HPP_
...
namespace sferes {
    namespace stat {
		...
		plt::figure();
		plt::figure_size(3000,1000);
		plt::subplot(1,2,1);
		plt::scatter(x,yaw,z,3.0, true);
		...
		plt::subplot(1,2,2);
		plt::scatter(y,yaw,z,3.0, true);
		// plt::quiver(x,y,v,w,z);

		// Add graph title
		plt::title(std::string("Archive ")+std::to_string(ea.gen()));
		plt::xlabel("Lateral Velocity");
    	plt::ylabel("Yaw Velocity");
		// Enable legend.
		plt::legend();
