#ifndef STAT_QD_PROGRESS_FIT_HPP_
#define STAT_QD_PROGRESS_FIT_HPP_
...
namespace sferes {
    namespace stat {
        SFERES_STAT(QdProgressFit, Stat){
        public:
            ...
            void _write_progress(const std::string& prefix, const EA& ea) const
            {
                ...
                // double counter[100][100]{};//to measure effective size of grid.


                for (auto it = ea.pop().begin(); it != ea.pop().end(); ++it) {

                    low_quant_fit((*it)->fit().value());
                    med_quant_fit((*it)->fit().value());
                    upp_quant_fit((*it)->fit().value());
