#ifndef __CONTROLLER_FIXED_HPP__
#define __CONTROLLER_FIXED_HPP__
...
namespace hexapod_controller {
          ...
          void set_parameters(const Eigen::VectorXd& ctrl)
          {
                  ...
                  case 0:
                          ...
                          if (_fixed) {
                                  ...
                                  _legs0commands.push_back(_dist_control_signal(ctrl[0], ctrl[1], ctrl[2]));
                  ...
                  case 1:
                          ...
                          if (_fixed) {
                                  ...
                                  _legs1commands.push_back(_dist_control_signal(ctrl[0], ctrl[1], ctrl[2]));
                  ...
                  case 2:
                          ...
                          if (_fixed) {
                                  ...
                                  _legs2commands.push_back(_dist_control_signal(ctrl[0], ctrl[1], ctrl[2]));
                  ...
                  case 3:
                          ...
                          if (_fixed) {
                                  ...
                                  _legs3commands.push_back(_dist_control_signal(ctrl[0], ctrl[1], ctrl[2]));
                  ...
                  case 4:
                          ...
                          if (_fixed) {
                                  ...
                                  _legs4commands.push_back(_dist_control_signal(ctrl[0], ctrl[1], ctrl[2]));
                  ...
                  case 5:
                          ...
                          if (_fixed) {
                                  ...
                                  _legs5commands.push_back(_dist_control_signal(ctrl[0], ctrl[1], ctrl[2]));
  ...
  protected:
        ...
        array_t _dist_control_signal(double init_ampl, double distance, double duty_cycle) const
        {
            array_t temp;
            double phase = 0.0;//no phase since we want the leg to stop at the given distance;
        //     duty_cycle = 0.5;//fixing here, hack,needs to be fixed

            double amplitude = 0.0;
            amplitude = init_ampl+(distance*2-1);//normalization since distance is between 0 and 1 at input
            if(std::abs(amplitude)>1.0){
                amplitude = std::max(-1.0, std::min(amplitude, 1.0));
            }
            amplitude = -1.0 * amplitude; //we want to start from the opposite amplitude to leverage duty cycle.


            int up_time = ARRAY_DIM * duty_cycle;
            int adjustment = ARRAY_DIM * (duty_cycle/3);

            double grad = (amplitude-init_ampl)/adjustment;

        //     std::cout<<amplitude<<"  "<<init_ampl<<"  "<<distance<<"  "<<grad<<std::endl;


            for (int i = 0; i < up_time; i++)
            {
                if(i<adjustment){
                temp[i] = init_ampl+i*grad;//linear approximation from initial position
                }
                else{
                temp[i] = amplitude;
                }
        ...
        //     std::cout<<"Break"<<std::endl;
            ...
            for (int i = 0; i < ARRAY_DIM; i++) {
                ...
                for (int d = 1; d <= kernel_size; d++) {
                    if (i - d < 0)
                        // command[i] += init_ampl * kernel[kernel_size - d];
                        command[i] += temp[ARRAY_DIM + i - d] * kernel[kernel_size - d];
                    else
                        command[i] += temp[i - d] * kernel[kernel_size - d];
                }
                ...
                for (int d = 1; d <= kernel_size; d++) {
                    if (i + d >= ARRAY_DIM)
                        // command[i] += (-amplitude) * kernel[kernel_size + d];
                        command[i] += temp[i + d - ARRAY_DIM] * kernel[kernel_size + d];
                    else
                        command[i] += temp[i + d] * kernel[kernel_size + d];
                }
